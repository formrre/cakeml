...
val _ = Datatype `
  tag = BlockTag num | RefTag | BytesTag bool num | NumTag bool | WordTag`;
...
Proof
 rpt strip_tac \\ fs[v2mw_def,bitstringTheory.length_pad_right]
 \\ simp[LENGTH_n2mw]
 \\ Cases_on `v2n v = 0` \\ fs[]
 \\ simp[v2n_def,numposrepTheory.num_from_bin_list_def]
 \\ Q.PAT_ABBREV_TAC `L = bitify _ _`
 \\ ASSUME_TAC (Q.SPECL [`2`,`L`] numposrepTheory.LOG_l2n_dropWhile)
 \\ fs[]
 \\ `EVERY ($> 2) L` by simp[Abbr`L`,every_bit_bitify]
 \\ fs[]
 \\ fs[v2n_eq0]
 \\ `EXISTS (λy. 0 ≠ y) L` by (simp[Abbr`L`] \\ fs[v2n_def,bitify_reverse_map]
     \\ fs[EXISTS_REVERSE,EXISTS_MAP]
     ...
     \\ fs[GENLIST_K_EVERY]
     \\ fs[o_DEF] \\ pop_assum mp_tac
    \\ Q.MATCH_ABBREV_TAC `EXISTS X _ ==> _`
    \\ `X = I` by (UNABBREV_ALL_TAC \\ simp[I_DEF,S_DEF,K_DEF] \\ ABS_TAC \\ TOP_CASE_TAC \\ simp[])
    \\ fs[])
 \\ fs[]
 \\ simp[GSYM ADD1]
 ...
 \\ simp[backend_commonTheory.ROUNDUP_DIV_def]
 \\ TOP_CASE_TAC \\ fs[]
 >- (simp[bitify_reverse_map] \\ simp[dropWhile_MAP]
     \\ Q.PAT_ABBREV_TAC `X = _ o _`
     \\ fs[o_DEF]
...
val small_word_def = Define `
  small_word (:'a) (w:bool list) <=>
    LENGTH w <= dimindex(:'a)-2`
val _ = export_rewrites["small_word_def"]
...
val v_inv_def = tDefine "v_inv" `
  ...
  (v_inv (Word w) (x,f,tf,heap:'a ml_heap) <=>
    if small_word (:'a) w then (x = Data (Word (word_lsl (v2w w) (dimindex(:'a) - LENGTH w)))) else
    (?ptr. (x = Pointer ptr (Word 0w)) /\
          (heap_lookup ptr heap = SOME (WordRep (:'a) w)))) /\
...
val v_inv_related = Q.prove(
  ...
  THEN1
   (full_simp_tac (srw_ss()) [v_inv_def,ADDR_APPLY_def,BlockRep_def]
    \\ TOP_CASE_TAC \\ full_simp_tac std_ss []
    THEN1 full_simp_tac (srw_ss())[ADDR_APPLY_def,get_refs_def]
    \\ full_simp_tac std_ss [gc_related_def] \\ res_tac
    \\ full_simp_tac (srw_ss())[] \\ NTAC 3 (POP_ASSUM MP_TAC)
    ...
    \\ res_tac
    \\ ntac 2 strip_tac
    \\ qexists_tac `g ' ptr` \\ conj_tac >- simp[ADDR_APPLY_def]
...
val v_inv_SUBMAP = Q.prove(
  ...
  THEN1 (
    rw[] \\ fs[v_inv_def]
    \\ qspecl_then[`:'a`,`l`]strip_assume_tac WordRep_DataElement
    \\ fs[]
    \\ imp_res_tac heap_store_rel_lemma \\ TOP_CASE_TAC \\ fs[] )
...
Proof
  rw [BlockRep_def,isDataElement_def,Bignum_def,i2mw_def,
    WordRep_def,RefBlock_def,Bytes_def]
QED
...
Proof
  ...
  \\ fs [case_eq_thms]
  \\ Cases_on `sp` \\ fs []
  \\ fs [heap_lookup_def, Bignum_def, WordRep_def, BlockRep_def]
  \\ rpt (pairarg_tac \\ fs [])
  \\ unlength_tac [ADD1]
  \\ IF_CASES_TAC \\ fs []
  \\ TRY (qexists_tac `xs`)
...
Theorem word_alt_thm:
   abs_ml_inv conf (ws ++ stack) refs (rs ++ roots,heap,be,a,sp,sp1,gens) limit ts ∧
   LENGTH ws = LENGTH rs ∧
   ~(small_word (:'a) w) ∧
   (WordRep (:'a) w :'a ml_el) = DataElement [] len (WordTag,xs) ∧
   LENGTH xs < sp
   ⇒
   ∃heap2.
     heap_store_unused_alt a (sp + sp1) (WordRep (:'a) w) heap = (heap2,T) ∧
...
Proof
  ...
  \\ `el_length wr = len + 1`
  by ( fs[Abbr`wr`] \\ rw[] \\ fs[el_length_def])
  \\ `LENGTH xs = len`
  by (
  ...
  \\ disch_then(qspec_then`WordRep(:'a)w`mp_tac)
...
val word_payload_def = Define `
  ...
  (word_payload ys l (RefTag) qs conf =
     ...
     (make_header conf 2w (LENGTH ys),
      MAP (word_addr conf) ys,
      (qs = []) /\ (LENGTH ys = l))) /\
  (word_payload ys l WordTag qs conf =
...
Theorem EXP_DIV:
   !e x y. 0<e ==> (e ** x) DIV (e ** y) = if y <= x \/ e = 1 then e ** (x - y) else 0
Proof
  rw[] \\ Cases_on `e = 1` \\ fs[]
  >- (IMP_RES_TAC LESS_EQ_ADD_EXISTS \\ POP_ASSUM (ASSUME_TAC o GSYM)
      \\ fs[]
      \\ simp[EXP_ADD]
      \\ Q.MATCH_ABBREV_TAC `_ DIV X = _`
      \\ `0 < X` by fs[Abbr`X`]
      \\ fs[MULT_DIV])
  \\ Cases_on `y = 0` \\ fs[]
  \\ `~(e ** y = 0)` by fs[]
  \\ `~(e ** y = 1)` by fs[]
  \\ `1 < e ** y` by DECIDE_TAC
  \\ fs[NOT_LESS_EQUAL] \\ clean_tac
  \\ simp[DIV_EQ_0]
QED

Theorem DIV_SMALL_SUB_MOD0:
   !x y d. 1 < d /\ 0 < y /\ y < d /\ x MOD d = 0 ==> (x - y) DIV d = x DIV d - 1
Proof
  rw[] \\ Cases_on `x = 0` >- fs[ZERO_DIV]
  \\ MATCH_MP_TAC DIV_UNIQUE
  \\ qexists_tac `d-y`
  \\ simp[]
...
local
 open extra_numTheory
in
Theorem EXP_SUB_DIV:
   !e x y z. 1<e /\ y < e ** z ==> ((e ** x - y) DIV (e ** z) = if x < z then 0 else (e ** (x - z) - (if y=0 then 0 else 1)))
Proof
  rw[] \\ fs[]
  \\ simp[EXP_DIV]
  >- (`~(e ** z = 0)` by fs[]
      ...
      \\ `1 < e ** z` by DECIDE_TAC
      \\ fs[DIV_EQ_0]
      \\ `e ** x < e ** z` suffices_by DECIDE_TAC
      \\ simp[EXP_MONO]
    )
  \\ simp[EXP_DIV]
  \\ ASSUME_TAC (Q.SPECL [`e ** x`,`y`,`e ** z`] DIV_SMALL_SUB_MOD0)
  \\ rfs[]
  \\ `e ** x MOD e ** z = 0` by (simp[MOD_EQ_0_DIVISOR] \\ qexists_tac `e ** (x-z)` \\ simp[]
...
Theorem small_word_and_neg2:
   (good_dimindex(:'a) /\ small_word (:'a) w) ==>
   -2w && (v2w w << (dimindex (:α) - LENGTH w))
    = ((v2w w):('a word)) << (dimindex (:α) - LENGTH w)
Proof
   rw[] \\ simp[word_lsl_def,word_and_def,fcpTheory.CART_EQ,fcpTheory.FCP_BETA]
   \\ rpt strip_tac
   \\ eq_tac \\ rw[]
   \\ rename1 `_ ' i`
   \\ Cases_on `i`
   >- (EVAL_TAC \\ fs[labPropsTheory.good_dimindex_def,dimword_def] \\ EVAL_TAC
       \\ fs[small_word_def])
   \\ rw[] \\ fs[] \\ EVAL_TAC
   \\ fs[dimword_def]
   \\ simp[n2w_def]
   ...
   \\ simp[MOD_2] \\ simp[case_eq_thms]
   \\ simp[GSYM ODD_EVEN]
   \\ simp[ODD_EXISTS]
   \\ fs[]
   \\ `(2 ** dimindex (:α) - 2) DIV 2 ** SUC n = (2 ** (dimindex(:'a) - 1)-1) DIV 2 ** n` by
    (ASSUME_TAC (Q.SPECL[`2`,`dimindex(:'a)`,`2`,`SUC n`] EXP_SUB_DIV)
     \\ Cases_on `n=0`
     >- fs[]
     \\ Cases_on `n=1`
     >- (fs[] \\ fs[good_dimindex_def])
     \\ `1 < SUC n` by fs[]
     \\ fs[]
     \\ ASSUME_TAC (Q.SPECL[`2`,`dimindex(:'a) -1`,`1`,`n`] EXP_SUB_DIV)
     \\ POP_ASSUM MP_TAC
     \\ `1<2` by simp[]
     \\ ASM_SIMP_TAC (srw_ss())[]
     \\ `1 < 2 ** n` by (Cases_on `n` >- (REWRITE_TAC[EXP] \\ fs[])
                         \\ rename1 `2 ** k` \\ Cases_on `2 ** k` \\ fs[]
                         \\ Cases_on `n` \\ fs[] \\ clean_tac
                         \\ rename1 `2 ** k = SUC m` \\ Cases_on `m` \\ fs[])
     \\ ASM_SIMP_TAC(srw_ss())[]
     \\ clean_tac
     \\ `0<n` by DECIDE_TAC \\ ASM_SIMP_TAC(srw_ss())[]
     \\ `~(dimindex (:α) < 1 + n)` by DECIDE_TAC \\ ASM_SIMP_TAC(srw_ss())[]
     \\ rpt strip_tac \\ rpt (pop_assum kall_tac)
     \\ simp[ADD1])
   \\ fs[]
   \\ Q.EXISTS_TAC ` (2 ** (dimindex (:α) - 2) - 2) DIV 2 ** n`
   \\ ASSUME_TAC (Q.SPECL[`2`,`dimindex(:'a)-1`,`1`,`n`] EXP_SUB_DIV)
   \\ POP_ASSUM mp_tac
   \\ `1<2` by simp[] \\ ASM_SIMP_TAC(srw_ss())[]
   \\ `1 < 2 ** n` by (Cases_on `n` >- (REWRITE_TAC[EXP] \\ fs[])
                         \\ rename1 `2 ** k` \\ Cases_on `2 ** k` \\ fs[]
                         \\ Cases_on `n` \\ fs[] \\ clean_tac
                         \\ rename1 `2 ** k = SUC m` \\ Cases_on `m` \\ fs[])
   ...
   \\ ASM_SIMP_TAC(srw_ss())[]
   \\ MP_TAC (Q.SPECL[`2`,`dimindex(:'a)-2`,`2`,`SUC a`]EXP_SUB_DIV)
   \\ ASM_SIMP_TAC(srw_ss())[]
   \\ Cases_on `a`
   >- (ASM_SIMP_TAC(srw_ss())[] \\ FULL_SIMP_TAC(srw_ss())[good_dimindex_def])
   \\ FULL_SIMP_TAC(srw_ss())[] \\ clean_tac
   \\ `1 < SUC (SUC n)` by DECIDE_TAC
   \\ FULL_SIMP_TAC(srw_ss())[] \\ pop_assum kall_tac
   \\ TOP_CASE_TAC \\ FULL_SIMP_TAC(srw_ss())[]
   >- simp[]
   \\ rpt strip_tac \\ ntac 2 (pop_assum kall_tac)
   \\ REWRITE_TAC[GSYM EXP]
   \\ REWRITE_TAC[LEFT_SUB_DISTRIB]
   \\ simp[ADD1]
   \\ `2 * 2 ** (dimindex (:'a) - (n + 4)) = 2 ** (dimindex(:'a) - (n+3))` by simp[Once (GSYM EXP)]
   \\ ASM_REWRITE_TAC[]
   \\ simp[EXP]
   \\ Q.PAT_ABBREV_TAC `X = dimindex(:'a) - _`
   \\ Q.PAT_ABBREV_TAC `Y = (2 ** X):num`
   \\ `~(Y = 0)` by simp[Abbr`Y`]
   \\ Cases_on `Y` \\ simp[]
   \\ rename1 `~(SUC z = 0)`
   \\ Cases_on `z` \\ simp[]
...
Theorem FDOM_tf_OR_specific:
        FDOM tf ⊆
             {ts' |
              ∃x.
                  ((∃l. ValueArray l ∈ FRANGE s.refs ∧ MEM x l) ∨
                   MEM x (MAP FST ws)) ∧ MEM ts' (v_all_ts x)}
        ==>
        FDOM tf ⊆
             {ts' |
              ∃x.
...
Theorem word_ml_inv_word:
   word_ml_inv (heap,be,a,sp,sp1,gens) limit ts c s.refs ws /\
    good_dimindex (:'a) /\
    small_word (:'a) w ==>
    word_ml_inv (heap,be,a,sp,sp1,gens) limit ts c s.refs
      ((Word w,Word ((word_lsl (v2w w) (dimindex(:'a) - LENGTH w)):'a word))::ws)
Proof
  fs [word_ml_inv_def,PULL_EXISTS] \\ rw[]
  \\ GEN_EXISTS_TAC "xs" `hs`
  \\ Q.REFINE_EXISTS_TAC `Data (Word v)`
  \\ simp[word_addr_def]
  \\ fs[abs_ml_inv_def]
  \\ fs[roots_ok_def]
  \\ Q.EXISTS_TAC `v2w w << (dimindex (:'a) - LENGTH w)`
  \\ simp[small_word_and_neg2]
  \\ conj_tac >- fs[]
  \\ fs[bc_stack_ref_inv_def]
  \\ Q.EXISTS_TAC `f` \\ fs[]
  \\ Q.EXISTS_TAC `tf` \\ fs[]
  ...
  >- (strip_tac
      ...
      \\ strip_tac
      \\ first_x_assum match_mp_tac
      >- (var_eq_tac \\ fs[get_refs_def])
      >- (Q.EXISTS_TAC `x` \\ Q.EXISTS_TAC `r` \\ fs[]))
  \\ simp[v_inv_def]
QED
...
(*
Theorem lsl_lsr_tight_bound:
   !(w:'a word) n. w << n >>> n = w <=> w2n w * 2 ** n < dimword(:'a)
Proof
   rw[] \\ reverse(eq_tac)
   >- (rw[] \\ match_mp_tac lsl_lsr \\ fs[])
   \\ rw[]
   \\ CCONTR_TAC
   \\ last_x_assum mp_tac \\ simp[]
   \\ simp[word_lsl_def]
   \\ simp[fcpTheory.CART_EQ]
   \\ simp[word_lsr_def,fcpTheory.CART_EQ,fcpTheory.FCP_BETA]
   \\ Q.REFINE_EXISTS_TAC`dimindex(:'a)-SUC j` \\ fs[]
   \\ simp[fcpTheory.FCP_BETA]
   \\ ...
QED

Theorem memory_rel_Word_IMP_tight_bound:
   memory_rel c be ts refs sp st m dm ((Word lw,v:'a word_loc)::vars) /\
   ~small_word(:'a) lw /\ good_dimindex(:'a) ==>
   (?P. P (:'a) (LENGTH (v2mw (:'a) lw)) c.len_size
     <=>
     decode_length c (make_header c 3w (LENGTH (v2mw (:'a) lw))) = n2w (LENGTH
     (v2mw (:'a) lw)):('a word))
Proof
   rw[memory_rel_def,decode_length_def,make_header_def]
   \\ Cases_on `dimindex(:'a) - c.len_size >= 4`
   >-(`3w >>> (dimindex(:'a) - c.len_size) = 0w:'a word` by (
         fs[word_eq_0,word_lsr_def] \\ rw[]
         \\ simp[fcpTheory.FCP_BETA]
         \\ rename1`i<c.len_size`
         \\ Cases_on`i<c.len_size` \\ fs[]
         \\ simp[n2w_def,fcpTheory.FCP_BETA]
       ) \\ fs[]
       \\ `3w << 2 = 12w` by (fs[good_dimindex_def] \\ rfs[] \\ EVAL_TAC \\ fs[])
       \\ fs[]
       \\ `12w >>> (dimindex(:'a) - c.len_size) = 0w:'a word` by (
         fs[word_lsr_def,word_eq_0]
         \\ rw[fcpTheory.FCP_BETA]
         \\ Cases_on`i<c.len_size` \\ fs[]
         \\ simp[n2w_def,fcpTheory.FCP_BETA]
       )
       \\ fs[]
       \\ simp[lsl_lsr_tight_bound]
       \\ qexists_tac`\x y z. 2 ** (dimindex x - z) * y MOD dimword x < dimword
       x` \\ simp[])
  \\ fs[heap_in_memory_store_def]
QED
...
Theorem memory_rel_Word_IMP_tight_bound:
   ...
   ~small_word(:'a) lw /\ good_dimindex(:'a) ==>
     ...
     (v2mw (:'a) lw)):('a word)
     <=>
     2 ** (dimindex (:'a) - c.len_size) * (LENGTH (v2mw (:'a) lw)) MOD dimword
     (:'a) < dimword (:'a))
Proof
  rw[memory_rel_def,decode_length_def,make_header_def]
   \\ Cases_on `dimindex(:'a) - c.len_size >= 4`
...
Theorem memory_rel_64_within_bound:
   memory_rel c be ts refs sp st m dm ((Word lw,v:'a word_loc)::vars) /\
   LENGTH lw = 64 /\ good_dimindex(:'a) ==>
     (decode_length c (make_header c 3w (LENGTH (v2mw (:'a) lw))) = n2w (LENGTH
     (v2mw (:'a) lw)):('a word))
Proof
   rw[memory_rel_def,decode_length_def,make_header_def]
   \\ reverse(Cases_on `dimindex(:'a) - c.len_size >= 4`)
   >- fs[heap_in_memory_store_def]
   \\ `3w >>> (dimindex(:'a) - c.len_size) = 0w:'a word` by (
         fs[word_eq_0,word_lsr_def] \\ rw[]
         \\ simp[fcpTheory.FCP_BETA]
   ...
   \\ fs[]
   \\ fs[LENGTH_v2mw]
   \\ fs[lsl_lsr_tight_bound]
   \\ fs[backend_commonTheory.ROUNDUP_DIV_def]
   \\ `64 MOD dimindex(:'a) = 0` by (fs[good_dimindex_def])
   \\ fs[]
   \\ fs[good_dimindex_def]
   >-(`2 MOD dimword(:'a) = 2` by fs[dimword_def] \\ fs[]
      \\ fs[heap_in_memory_store_def]
      \\ simp[GSYM EXP]
   ...
   *)
Theorem memory_rel_Word_IMP:
   memory_rel c be ts refs sp st m dm ((Word lw,v:'a word_loc)::vars) /\
   ~small_word (:'a) lw /\
   good_dimindex (:'a) ==>
   ?ff ptr x w.
     w = make_header c 3w (LENGTH (v2mw (:'a) lw)) /\
     decode_length c w = n2w (LENGTH (v2mw (:'a) lw)) /\
     LENGTH (v2mw (:'a) lw) < dimword(:'a) /\
     (word_list (x + bytes_in_word)
                (MAP Word (v2mw (:'a) lw)) * ff) (fun2set (m,dm)) /\
...
Proof
 ...
 \\ `X` by
  (UNABBREV_ALL_TAC \\
  fs[abs_ml_inv_def]
  \\ fs[heap_in_memory_store_def,bc_stack_ref_inv_def]
  \\ fs[v_inv_def]
  \\ imp_res_tac heap_lookup_SPLIT
  \\ fs[]
  \\ clean_tac
  \\ fs[word_heap_APPEND,word_heap_def,word_el_def,UNCURRY,word_list_def,WordRep_def]
  \\ full_simp_tac (std_ss++sep_cond_ss) [cond_STAR]
    \\ fs[word_payload_def]
  \\ Q.MATCH_ASMSUB_ABBREV_TAC`decode_length c X = Y`
  \\ Q.UNDISCH_TAC`decode_length c X = Y`
  \\ UNABBREV_ALL_TAC
  ...
  \\ fs[]
  \\ conj_tac >- (
      fs[abs_ml_inv_def,bc_stack_ref_inv_def,v_inv_def,heap_in_memory_store_def]
      \\ imp_res_tac heap_lookup_SPLIT
      \\ fs[]
      \\ clean_tac
      \\
  ...
  \\ Q.PAT_ABBREV_TAC`X=word_bit _ _`
  \\ Q.PAT_ABBREV_TAC`Y=~word_bit _ _`
  \\ Q.PAT_ABBREV_TAC`Z=word_bit _ _`
  \\ `X /\ Y /\ Z` by (UNABBREV_ALL_TAC
      \\ simp[make_header_def]
      \\ fs[abs_ml_inv_def,heap_in_memory_store_def]
      \\ full_simp_tac (srw_ss()++wordsLib.WORD_BIT_EQ_ss) []
      \\ fs[good_dimindex_def] \\ rfs[] \\ fs[] \\ EVAL_TAC \\ fs[])
  \\ fs[]
  ...
  \\ Q.EXISTS_TAC`ptr` \\ simp[]
  \\ clean_tac
  \\ fs[heap_in_memory_store_def]
  \\ drule (Q.SPECL[`st`,`ptr`](Q.SPEC`Word 0w` (GEN_ALL get_real_addr_get_addr)))
  \\ strip_tac
  \\ pop_assum (mp_tac o Q.SPECL[`curr`,`WordRep(:'a) lw`])
  \\ impl_tac >- fs[]
  \\ rw[]
  \\ imp_res_tac heap_lookup_SPLIT
  \\ fs[]
  ...
  \\ fs[word_payload_def]
  \\ SEP_R_TAC \\ fs[]
  \\ Q.MATCH_ASMSUB_ABBREV_TAC`X (fun2set (m,dm))`
  \\ Q.UNDISCH_TAC`X (fun2set (m,dm))`
  \\ UNABBREV_ALL_TAC
  \\ simp[AC STAR_COMM STAR_ASSOC]
  \\ SIMP_TAC (std_ss++sep_cond_ss)[SEP_CLAUSES]
  \\ ONCE_REWRITE_TAC[STAR_COMM]
  \\ Q.PAT_ABBREV_TAC `X=word_heap _ _ _ * word_heap _ _ _`
  \\ ONCE_REWRITE_TAC[STAR_COMM]
  \\ Q.PAT_ABBREV_TAC`Y=word_list _ _`
  \\ Q.MATCH_GOALSUB_ABBREV_TAC`X * Y *Z`
  \\ Q.MATCH_GOALSUB_ABBREV_TAC`X * Y * Z * V`
  \\ `X * Y * Z * V = (X * Z * V) * Y` by metis_tac[STAR_ASSOC,STAR_COMM]
  \\ ASM_REWRITE_TAC[]
  \\ strip_tac
  \\ asm_exists_tac \\ fs[]
QED
...
Theorem Word64Rep:
    !w.LENGTH w=64 /\ good_dimindex(:'a) ==> WordRep (:α) w =
         if dimindex (:α) < 64 then
           DataElement ([]:'a  word_loc heap_address list) 2
             (WordTag,[Word ((31 >< 0) ((v2w w):word64)); Word ((63 >< 32) ((v2w
             w):word64))])
         else DataElement [] 1 (WordTag,[Word ((63 >< 0) ((v2w w):word64))])
Proof
  reverse(rw[good_dimindex_def])
  >-(simp[WordRep_def]
     \\ conj_tac >- EVAL_TAC
     \\ simp[v2mw_def]
     \\ simp[Once n2mw_def]
     ...
     \\ TOP_CASE_TAC
     >-(
        simp[GSYM n2w_v2n] \\ EVAL_TAC
     )
     \\ simp[PAD_RIGHT]
     \\ `!x. 1 - SUC x = 0` by simp[]
     \\ ASM_REWRITE_TAC[]
     \\ conj_tac
     >-(`v2n w MOD dimword(:'a) = v2n w` by (simp[]
          \\ ASM_REWRITE_TAC[dimword_def]
          \\ `64 = LENGTH w` by fs[]
          \\ pop_assum (fn a=>SUBST_TAC[a])
          \\ simp[v2n_lt])
        \\ fs[]
        \\ simp[GSYM n2w_v2n,word_extract_n2w]
        \\ REWRITE_TAC[bitTheory.BITS_THM]
        \\ `2 ** (SUC 63 - 0) = dimword(:'a)` by fs[dimword_def]
        \\ pop_assum (fn a=>SUBST_TAC[a])
        \\ simp[]
     )
     ...
     \\ pop_assum mp_tac
     \\ simp[DIV_EQ_0]
     \\ `dimword(:'a) = 2 ** LENGTH w` by fs[dimword_def]
     \\ pop_assum(fn a => SUBST_TAC[a])
     \\ simp[v2n_lt]
  )
  \\ simp[WordRep_def]
...
Proof
  ...
  \\ TRY(fs[good_dimindex_def] \\ NO_TAC)
  \\ fs[word_addr_def]
  ...
  \\ simp[]
  \\ imp_res_tac heap_lookup_SPLIT
  \\ qspecl_then[`:'a`,`w64`]strip_assume_tac WordRep_DataElement
  \\ drule Word64Rep \\ impl_tac >- fs[]
  \\ strip_tac \\ fs[]
  \\ pop_assum kall_tac
...
Theorem memory_rel_Word_alt:
    ...
    ~(small_word (:'a) w) ∧
    (WordRep (:'a) w : 'a ml_el) = DataElement [] (LENGTH ws) (WordTag,ws) ∧
   LENGTH ws < sp ∧
   ...
   ∃ne curr m1.
     ...
     memory_rel c be ts refs (sp - (LENGTH ws + 1))
        ...
        ((Word w, make_ptr c (ne - curr) (0w:'a word) (LENGTH ws))::vars)
Proof
  ...
  \\ fs[] \\ clean_tac
  \\ drule (GEN_ALL word_alt_thm) \\ fs[]
  \\ disch_then drule
  \\ strip_tac
  \\ pop_assum (mp_tac o Q.ISPECL[`ws:(('a word_loc) list)`,`LENGTH (ws:('a word_loc) list)`])
...
val memory_rel_WordOp_alt =
  memory_rel_Word_alt |> Q.GEN`vs` |> Q.SPEC`[w1;w2]`
  |> CONV_RULE(LAND_CONV(SIMP_CONV(srw_ss())[]))
  |> curry save_thm "memory_rel_WordOp_alt"
...
Theorem memory_rel_RefByte_alt_noinit2:
  memory_rel c be ts refs sp st m dm vars ∧
   new ∉ FDOM refs ∧ byte_len (:'a) n < sp ∧
   byte_len (:'a) n < 2 ** (dimindex (:α) − 4) /\
   byte_len (:'a) n < 2 ** c.len_size /\
...
Proof
  ...
  \\ pop_assum SUBST_ALL_TAC
  \\ qmatch_goalsub_abbrev_tac`Word (make_byte_header c fl n) :: X`
  \\ GEN_EXISTS_TAC"ws"`X`
  \\ qpat_abbrev_tac `ws2 = Word (make_byte_header c fl n)::_`
  \\ qpat_abbrev_tac `ws1 = Word (make_byte_header c fl n)::_`
  \\ `ws1 = ws2` by (
    unabbrev_all_tac \\ fs [map_replicate]
    \\ Q.MATCH_ABBREV_TAC`Y=_`
    \\ ...
    )
  \\ rveq \\ fs []
  \\ simp_tac (std_ss++helperLib.sep_cond_ss) [cond_STAR,GSYM CONJ_ASSOC]
...
QED
*)
Theorem heap_length_DataElement:
  !xs l data. heap_length [DataElement xs l data] = l + 1
Proof
    rw[] \\ EVAL_TAC
QED

Theorem memory_rel_RefByte_alt_noinit3:
  memory_rel c be ts refs sp st m dm vars ∧
   new ∉ FDOM refs ∧ byte_len (:'a) n < sp ∧
   byte_len (:'a) n < 2 ** (dimindex (:α) − 4) /\
   byte_len (:'a) n < 2 ** c.len_size /\
   good_dimindex (:α) /\
   LENGTH arr = n-(n MOD (dimindex(:'a) DIV 8)) ⇒
   ∃free curr m1 ws.
     FLOOKUP st NextFree = SOME (Word free) ∧
     FLOOKUP st CurrHeap = SOME (Word curr) ∧
     LENGTH ws = byte_len (:'a) n /\
     (let w' = bytes_in_word * (n2w (byte_len (:'a) n + 1)):'a word in
      ...
      let ws = LUPDATE (Word (last_bytes nb 0w 0w 0w be)) (byte_len (:'a) n - 1) ws in
        store_list free (Word (make_byte_header c fl n)::ws) m dm = SOME m1 ∧
        memory_rel c be ts (refs |+ (new,ByteArray fl (arr++ REPLICATE nb 0w)))
          (sp − (byte_len (:'a) n + 1)) (st |+ (NextFree,Word (free + w'))) m1 dm
          ((RefPtr new,make_ptr c (free − curr) 0w (byte_len (:'a) n))::vars))
Proof
  simp_tac std_ss [LET_THM]
  ...
  \\ drule (GEN_ALL new_byte_alt_thm)
  \\ disch_then (qspecl_then [`(byte_len (:'a) n)`,
        `new`,`fl`,`arr ++ REPLICATE (n MOD (dimindex(:'a) DIV 8)) 0w`] mp_tac)
  \\ fs [LENGTH_REPLICATE]
  \\ `n-n MOD (dimindex(:'a) DIV 8) + n MOD (dimindex(:'a) DIV 8) = n`
    by(match_mp_tac SUB_ADD \\ match_mp_tac MOD_LESS_EQ
       \\ fs[good_dimindex_def]
  ...
  \\ conj_tac THEN1
    ...
    \\ fs [dimword_def,DIV_LT_X])
 \\ simp[last_bytes_0]
 \\ full_simp_tac (std_ss++helperLib.sep_cond_ss) [cond_STAR]
 \\ fs[heap_length_DataElement]
 \\ Q.MATCH_ASMSUB_ABBREV_TAC`X (fun2set (m,dm))`
 \\ Q.UNDISCH_TAC`X (fun2set (m,dm))`
 \\ simp[Abbr`X`]
 \\ Q.MATCH_GOALSUB_ABBREV_TAC`(X * Y * Z * A * B) (fun2set (m,dm))`
 \\ Q.PAT_ABBREV_TAC`TZ = word_list_exists _ _`
 \\ `TZ = Z` by (UNABBREV_ALL_TAC \\ MK_COMB_TAC \\ simp[]
                 \\ MK_COMB_TAC \\ simp[]
                 \\ Q.MATCH_ABBREV_TAC`(X:'a word) + Y=(A:'a word) + B + C`
                 \\ `Y = B + C` suffices_by simp[]
                 \\ UNABBREV_ALL_TAC
                 \\ REWRITE_TAC[GSYM WORD_LEFT_ADD_DISTRIB]
                 \\ simp[word_add_n2w])
 \\ simp[Abbr`TZ`]
 \\ Q.PAT_ABBREV_TAC`TA = word_heap _ _ _`
 \\ `TA = A` by (UNABBREV_ALL_TAC
       \\ MK_COMB_TAC \\ simp[]
       \\ MK_COMB_TAC \\ simp[]
       \\ MK_COMB_TAC \\ simp[]
       \\ Q.MATCH_ABBREV_TAC`(X:'a word) + Y=(A:'a word) + B + C`
       ...
       \\ simp[word_add_n2w])
 \\ simp[Abbr`TA`]
 \\ `X * Y * Z * A * B = Y * (X * Z * A * B)` by
 (metis_tac[STAR_COMM,STAR_ASSOC])
 \\ pop_assum (fn a=>SUBST_TAC[a])
 \\ Q.PAT_ABBREV_TAC`Y2 = word_list _ _`
 \\ `X * Y2 * Z * A * B = Y2 * (X * Z * A * B)` by
 (metis_tac[STAR_COMM,STAR_ASSOC])
 \\ pop_assum (fn a=>SUBST_TAC[a])
 \\ simp[Abbr`X`,Abbr`Z`,Abbr`A`,Abbr`B`]
 \\ Q.PAT_ABBREV_TAC`X = _ * _ * _ * _`
 \\ simp[Abbr`Y`,Abbr`Y2`]
 \\ Q.PAT_ABBREV_TAC`Y=Word _ :: _`
 \\ `byte_len (:'a) n + 1 = LENGTH Y` by (
     simp[Abbr`Y`]
 )
 \\ simp[]
...
Proof
  ...
  \\ impl_tac >- ( fs[] \\ clean_tac
                   \\ fs[word_heap_def] \\ fs[heap_length_def]
                   \\ fs[Abbr`r`,Abbr`bw`]
                   \\ fs[shift_def]
                   \\ Cases_on `bs` \\ fs[]
                   \\ `SUC (LENGTH t) + (2 ** shift (:'a) - 1) = LENGTH t + 2 ** shift (:'a)`
                     by (Cases_on `2**shift(:'a)` \\ simp[] >-(Cases_on `n` \\ fs[])
                         >- fs[shift_def] \\ fs[])
                   \\ fs[]
                   \\ pop_assum kall_tac
                   \\ fs[el_length_def]
...
val v_same_type_def = tDefine"v_same_type"`
  ...
  (v_same_type (Word v) (Word w) = (LENGTH v = LENGTH w)) ∧
...
Theorem large_word_memory_rel_simple:
   memory_rel c be ts refs sp st m dm ((Word wl,v:'a word_loc)::vars) /\
   ~small_word (:'a) wl /\
   good_dimindex (:'a) ==>
   ?ptr x w.
    v = Word (get_addr c ptr (Word 0w)) /\
...
Theorem word_list_SPLIT:
   !x ff m dm gg a b. LENGTH a = LENGTH b /\ (word_list x a * ff)(fun2set (m,dm)) /\ (word_list x b * gg)(fun2set (m,dm))
   ==>
   a = b
Proof
Induct_on`a` \\ fs[word_list_def,SEP_CLAUSES]
\\ Induct_on`b` \\ fs[word_list_def,SEP_CLAUSES]
\\ rw[] \\ fs[]
\\ SEP_R_TAC
\\ first_x_assum match_mp_tac
\\ fs[]
\\ qexists_tac`x+bytes_in_word`
\\qexists_tac`one(x,h)*ff`
\\qexists_tac`m`
\\qexists_tac`dm`
\\qexists_tac`one(x,h)*gg`
...
Theorem PAD_LEFT_11:
  !c n x y. (x<>[] /\ y<>[] /\ HD x <> c /\ HD y <> c)
  ==>
   (PAD_LEFT c n x = PAD_LEFT c n y <=> x = y)
Proof
  rw[] \\ simp[PAD_LEFT]
  \\ rw[]
  \\ Cases_on`LENGTH x = LENGTH y` \\ fs[]
  \\ simp[LIST_EQ_REWRITE]
  \\ Cases_on`n-LENGTH x + LENGTH x = n - LENGTH y + LENGTH y` \\ fs[]
  \\ reverse(Cases_on `LENGTH x<n` \\ fs[])
  >- (`n-LENGTH x=0` by DECIDE_TAC
         \\ ASM_REWRITE_TAC[] \\ fs[]
         \\ `n- LENGTH y<>0` by DECIDE_TAC
         \\ Cases_on`n=0` \\ fs[]
         \\ qexists_tac`0` \\ fs[]
         \\ Cases_on`n-LENGTH y` \\ fs[]
         \\ simp[GENLIST_K_REVERSE_SUC])
  \\ Cases_on`n<LENGTH y` \\ fs[]
  \\ Cases_on`n=LENGTH y` \\ fs[NOT_LESS]
  >-(qexists_tac`0`
     \\ Cases_on`LENGTH y - LENGTH x` >- fs[]
     \\ conj_tac >- (Cases_on`y` \\ fs[])
     \\ simp[EL_APPEND_EQN])
  \\ `LENGTH y<n` by DECIDE_TAC \\ fs[]
  \\ clean_tac
  \\ Cases_on`n-LENGTH x < n-LENGTH y`
  >-(qexists_tac`n-LENGTH x`
     \\ simp[EL_APPEND_EQN])
  \\ qexists_tac`n-LENGTH y` \\ simp[EL_APPEND_EQN]
QED

Theorem PAD_RIGHT_11:
  !c n x y. (x<>[] /\ y<>[] /\ LAST x <> c /\ LAST y <> c)
  ==>
   (PAD_RIGHT c n x = PAD_RIGHT c n y <=> x = y)
Proof
  rw[] \\ simp[PAD_RIGHT]
  \\ rw[]
  \\ Cases_on`LENGTH x = LENGTH y` \\ fs[]
  \\ simp[LIST_EQ_REWRITE]
  \\ Cases_on`n-LENGTH x + LENGTH x = n - LENGTH y + LENGTH y` \\ fs[]
  \\ reverse(Cases_on`LENGTH y<n` \\ fs[])
  >- (`n-LENGTH y = 0` by DECIDE_TAC
      \\ `n-LENGTH y + LENGTH y =LENGTH y` by (ASM_REWRITE_TAC[] \\ simp[])
      \\ ASM_REWRITE_TAC[]
      \\ pop_assum(fn a=>fs[a])
      \\ Cases_on`n<LENGTH x` \\ fs[]
      \\ `LENGTH x<=LENGTH y` by fs[]
      \\ clean_tac
      \\ `LENGTH x < LENGTH y` by fs[]
      \\ fs[] \\ clean_tac
      \\ qexists_tac`LENGTH y -1` \\ fs[]
      \\ fs[EL_APPEND_EQN]
      \\ fs[GSYM ELL]
      \\ simp[EL_ELL])
  \\ Cases_on`n<LENGTH x` \\ fs[]
  ...
  >-(qexists_tac`LENGTH x - 1`
     \\ Cases_on`LENGTH x - LENGTH y` >- fs[]
     \\ conj_tac >- (Cases_on`x` \\ fs[])
     \\ simp[EL_APPEND_EQN]
     \\ fs[GSYM ELL,EL_ELL])
  \\ clean_tac
  \\ Cases_on`LENGTH x < LENGTH y` \\ fs[]
  >-(qexists_tac`LENGTH y-1` \\ simp[EL_APPEND_EQN]
     \\ fs[GSYM ELL,EL_ELL])
  \\ qexists_tac`LENGTH x-1` \\ simp[EL_APPEND_EQN]
  \\ fs[GSYM ELL,EL_ELL]
QED
...
Theorem n2mw_exists_non0:
  !n. n <> 0 ==> EXISTS(\x. x <> 0w) (n2mw n)
Proof
  ...
  \\ last_x_assum kall_tac
  \\ simp[Once n2mw_def]
  \\ Cases_on`n MOD dimword(:'a) = 0` \\ fs[]
  \\ Cases_on`n DIV dimword(:'a) = 0` \\ fs[]
  \\ simp[Once n2mw_def]
  \\ ASSUME_TAC DIMWORD_GT_1
  \\ fs[DIV_EQ_0]
QED

Theorem PAD_RIGHT_GENLIST:
  !c n l. LENGTH l <= n ==>
  PAD_RIGHT c n l = GENLIST (\i. if i < LENGTH l then EL i l else c) n
Proof
  simp[PAD_RIGHT]
  \\ rw[]
  \\ `n = LENGTH l + (n-LENGTH l)` by simp[]
  \\ pop_assum (fn a=>GEN_REWRITE_TAC(ONCE_DEPTH_CONV o RAND_CONV)
  empty_rewrites [a])
...
!vptr. good_dimindex(:'a) /\
~small_word (:'a) w1 /\
LENGTH w1 = LENGTH w2 /\
memory_rel c be ts refs sp st m dm
((Word w1,(vptr:'a word_loc))::(Word w2,vptr)::vars) /\ good_dimindex (:α) ==>
  w1 = w2
Proof
   rw[]
   ...
   \\ rpt var_eq_tac \\ fs[]
   \\ clean_tac
   \\ fs[Once (GSYM v2mw_same_length_11)]
   \\ simp[Once (GSYM MAP_Word_11)]
   \\ match_mp_tac word_list_SPLIT
   \\ qexists_tac`x+bytes_in_word`
   \\ rename1 `word_list (x + bytes_in_word) (MAP Word (v2mw (:α) w2)) * gg`
   \\ rename1 `word_list (x + bytes_in_word) (MAP Word (v2mw (:α) w1)) * ff`
   \\ qexists_tac`ff`\\qexists_tac`m`\\qexists_tac`dm` \\ qexists_tac`gg`
   \\ fs[]
   \\ fs[LENGTH_v2mw]
QED
...
Theorem memory_rel_pointer_eq = Q.prove(`
  ...
  >- (
      ...
      >-(imp_res_tac memory_rel_swap
         ...
         \\ fs[]
         \\ rfs[]
         \\ ASSUME_TAC(Q.SPECL[`l'`,`l`]shifted_equality)
         \\ rw[]
         \\ rfs[]
      )
      \\ strip_tac \\ pop_assum kall_tac
      \\ Q.UNDISCH_TAC `l <> l'`
      \\ simp[]
      \\ match_mp_tac memory_rel_Word_eq
      \\ qexists_tac`w` \\ fs[]
  )
  >- (
    Cases_on `b` \\ fs []
...
val word_neq_Word = Q.prove(
 `!w1 w2. LENGTH w1 <= dimindex (:'a) - 2 /\
  ~((v2w w1):('a word) << (dimindex(:'a) - LENGTH w1) = (v2w w2):('a word) << (dimindex(:'a) - LENGTH w2)) /\ (LENGTH w1 = LENGTH w2)
  ==> ~(w1 = w2)`,
  rw[] \\ fs[word_eq,word_bit_lsl] \\ fs[bit_v2w]
   \\ rw[]
   \\ rveq \\ Cases_on `n<dimindex(:'a)` \\ fs[]
   \\ Cases_on `dimindex(:'a) <= n + LENGTH w2` \\ fs[]
   \\ simp[WORD_EQ]
   \\ fs[testbit_def]
   \\ Q.MATCH_ASSUM_ABBREV_TAC `~(field X X _ = [T] <=> field Y Y _ = [T])`
   \\ Q.UNABBREV_TAC `X`
   \\ fs[field_def,shiftr_def]
   \\ fs[fixwidth_def]
   \\ Cases_on `LENGTH w2 < X + 1` \\ fs[]
   >- (Cases_on `w1` \\ Cases_on `w2` \\ fs[]
       \\ fs[zero_extend_def]
       \\ every_case_tac
       >- ( fs[PAD_LEFT]
          \\ Cases_on `Y` \\ fs[]
          \\ rename1 `1 - (LENGTH aa - bb)`
          \\ Cases_on `LENGTH aa - bb` \\ fs[])
       \\ rename1 `SUC (LENGTH aa) - (Y+1)` \\ fs[ADD1]
       \\ rename1 `(~(x <=> y)) \/ _`
       \\ Cases_on `x = y` \\ fs[]
       \\ Cases_on `LENGTH aa - Y` \\ fs[]
       \\ clean_tac
       \\ fs[LIST_EQ_REWRITE]
       \\ fs[HD_DROP,EL_TAKE] \\ rename1 `LENGTH aa - Y = SUC n'`
       \\ qexists_tac `n'` \\ fs[])
   \\ Cases_on `LENGTH w2 < Y + 1` \\ fs[]
   >- (fs[zero_extend_def] \\ fs[PAD_LEFT] \\ Cases_on `LENGTH w2 - Y` \\ fs[])
   >- (fs[LIST_EQ_REWRITE] \\ fs[HD_DROP,EL_TAKE]
...
val word_eq_thm0 = prove(
  ...
  THEN1 (* do_eq Word *)
   (rw[] \\ fs[vb_size_def]
    ...
    \\ `small_word (:'a) w2 = small_word(:'a) w1` by fs[]
    \\ Cases_on`small_word (:'a) w1`
    >- (rename1`~word_bit 0 w1rep \/ ~word_bit 0 w2rep`
        \\ imp_res_tac memory_rel_small_word_IMP
        \\ fs[]
        \\ simp[word_lsl_def,Once word_bit_def,fcpTheory.FCP_BETA]
        \\ `~(dimindex (:α) <= LENGTH w2)` by (fs[good_dimindex_def] \\ DECIDE_TAC)
        \\ fs[]
        \\ drule memory_rel_tail
        \\ qhdtm_x_assum `memory_rel` kall_tac
        \\ rw[]
        \\ imp_res_tac memory_rel_small_word_IMP
        \\ fs[]
        \\ rfs[]
        \\ clean_tac
        \\ rpt_drule shifted_equality)
    \\ drule memory_rel_Word_IMP
    \\ fs[] \\ strip_tac
    \\ fs[]
    \\ simp[Once get_addr_def]
    \\ simp[get_lowerbits_def]
    \\ simp[word_bit_or]
    \\ `word_bit 0 1w` by EVAL_TAC
    \\ fs[]
    \\ drule memory_rel_tail
    \\ qhdtm_x_assum `memory_rel` kall_tac
    \\ strip_tac
    ...
    \\ simp[word_bit_or]
    \\ simp[get_lowerbits_def]
    \\ simp[word_header_def]
    \\ ntac 2 (ONCE_REWRITE_TAC[LENGTH_v2mw])
    \\ fs[]
    \\ fs[case_eq_thms]
    \\ mp_tac(Q.SPECL[`v2mw (:'a) w1`,`v2mw (:'a) w2`,`ff`,`ff'`] word_cmp_loop_thm)
    \\ fs[LENGTH_v2mw]
    \\ strip_tac
    \\ pop_assum(mp_tac o Q.SPECL[`x`,`x'`])
    \\ impl_tac >- fs[]
    \\ rw[]
    \\ fs[case_eq_thms]
    \\ rfs[]
    \\ clean_tac
    \\ `1 <> 2 MOD dimword(:'a)` by (fs[good_dimindex_def,dimword_def])
    \\ fs[]
    \\ FULL_CASE_TAC \\ fs[]
    >-(mp_tac(Q.SPECL[`v2mw(:'a) w1`,`v2mw(:'a) w2`] mw_cmp_thm)
       \\ impl_tac
       >-fs[LENGTH_v2mw]
       \\ fs[]
       \\ simp[v2mw_def]
       \\ simp[mw2n_PAD_RIGHT_0w]
       \\ simp[mw2n_n2mw]
    ...
    \\ impl_tac >- fs[LENGTH_v2mw]
    \\ rw[]
    \\ fs[v2mw_def,mw2n_n2mw,mw2n_PAD_RIGHT_0w]
    \\ drule v2n_same_length_11
    \\ fs[]
   )
  THEN1 (* do_eq RefPtr *)
